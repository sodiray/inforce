...
export const collect = async (targetPath, testFileRegex, testFnRegex, ignores) => {

  const filePaths = await collectAllFilesUnderPath(ignores)(targetPath)
...
export const collectAllFilesUnderPath = (ignores = []) => {
  const collect = async (path) => {
    const stat = await fse.stat(path)
    if (!stat.isDirectory()) return [{ path, filename: pathlib.basename(path) }]
    if (ignores.some(ignore => path.includes(ignore))) return []
    const dir = await fse.readdir(path)
    const results = await asyncMap(dir, file => collect(`${path}/${file}`))
